...
try:
    ...
    print("[PPF Solver] Required package 'cbor2' is missing. Starting automatic installation...")
    try:
        # 블렌더 자체 내장 파이썬 환경(sys.executable)에 pip로 cbor2 설치
        subprocess.run([sys.executable, "-m", "pip", "install", "cbor2"], check=True)
        print("[PPF Solver] 'cbor2' package installed successfully!")
    except Exception as e:
        print(f"[PPF Solver] Failed to install 'cbor2' automatically (administrator privileges may be required): {e}")
# ----------------------------------------------------------------
